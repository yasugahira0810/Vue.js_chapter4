...
- ルート定義に{ path: '/a', redirect: '/b' }と書くことでpathで指定したパスにアクセスした時にredirectで指定したパスに遷移する
- Not Foundページを作る時に便利
...
- URL上はアクセスした時のものを保持した状態で、別のルートで定義したパスへ遷移する
- { path: '/a', component: A, alias: '/b' }と書くことで、aliasで指定したパスにアクセスした時にURL上は/bのまま、コンポーネントAがレンダリングされ、/aへアクセスしたかのように振る舞う。
- *リダクレクトがpathからredirectへの遷移だったのに対して、エイリアスはaliasからpathへの遷移なので、混乱しないよう注意する。*
